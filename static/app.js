(() => {
  ...
  async function openExpertise(preselect) {
    ...
    function grid() {
      ...
      tpls.forEach(function (t) {
        ...
        b.innerHTML = '<span class="exp-ico">' + t.emoji + '</span><span class="exp-label">' + escapeHtml(t(t.label)) +
    ...
    function detail(t) {
      body.innerHTML =
        '<button class="exp-back" type="button">' + t("\u2190 Mbrapa") + '</button>' +
        '<div class="exp-title">' + t.emoji + ' ' + escapeHtml(t(t.label)) + '</div>' +
        _pl(t("Elementet q\u00eb duhen provuar"), t.elements) +
        _pl(t("Provat tipike"), t.evidence) +
        _pl(t("Mbrojtjet / pikat e dob\u00ebta"), t.defenses) +
        _pl(t("Afatet"), t.deadlines) +
        _pl(t("Pyetje udh\u00ebzuese"), t.questions) +
        '<div class="exp-attachrow"><label class="ac-attach">' + t("\ud83d\udcce Bashkëngjit PDF/foto") + '<input type="file" class="exp-file" accept=".pdf,.jpg,.jpeg,.png,.webp,.svg,.tif,.tiff" hidden></label></div>' +
        '<textarea class="ac-ta exp-ta" placeholder="' + t("Përshkruaj faktet e çështjes (ose bashkëngjit dokumentet)…") + '"></textarea>' +
        '<div class="ac-row"><button class="ac-run exp-run" type="button">' + t("Gjenero ekspertiz\u00ebn \u2192") + '</button><span class="ac-status exp-status"></span></div>' +
      ...
      run.onclick = async function () {
        ...
        if (facts.length < 15) { status.textContent = t("P\u00ebrshkruaj faktet."); return; }
        run.disabled = true; status.textContent = t("Po nd\u00ebrtoj ekspertiz\u00ebn me dy mendjet\u2026 (~3-4 min)"); result.innerHTML = "";
        try {
  ...
  async function openNotaryDeed() {
    ...
    kind.innerHTML = types.map(function (t) { return '<option value="' + t.key + '">' + t.emoji + ' ' + escapeHtml(t(t.label)) + '</option>'; }).join("");
    function showMust() {
  ...
  async function openAfati() {
    ...
    trig.innerHTML = triggers.map(function (t) { return '<option value="' + t.key + '">' + escapeHtml(t(t.label)) + '</option>'; }).join("");
    run.onclick = async function () {
