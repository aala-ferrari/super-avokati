(() => {
  ...
  async function uploadFiles(files) {
    ...
    const _limite = (f) => {
      const n = f.name || "";
      if (VIDEO_EXT.test(n)) return MAX_VIDEO;
      if (AUDIO_EXT.test(n)) return MAX_AUDIO;
      return MAX_DOC;
    };
    const troppoGrandi = files.filter((f) => f.size > _limite(f));
    if (troppoGrandi.length) {
      _dossierAvviso(TT("Skedar shumë i madh (max 25 MB dokument / 200 MB audio / 500 MB video): ") +
  ...
  function renderCitationsBadge(payload, decPayload) {
    ...
    wrap.innerHTML = `
      <summary>
        ...
        <span class="cit-hint">${_t("klik për detaje")}</span>
      </summary>
      <ul class="cit-list">
        ${items.map((c) => {
          if (c.status === "verified") {
            const head = c.article_heading ? ` — ${escapeHtml(c.article_heading.slice(0, 60))}` : "";
            const fresh = (c.volatility || "").toUpperCase() === "MEDIUM" ? ` <span class="cit-fresh" title="${_t("Kjo dispozitë ndryshon herë pas here — verifiko versionin aktual (Ligj i gjallë)")}">⏳</span>` : "";
          ...
          if (c.status === "fake") {
            return `<li class="cit-row cit-row-fake">
              <span class="cit-status">✗</span>
              <code>${escapeHtml(c.raw)}</code>
              <span class="cit-meta">${_t("nuk u gjet")} ${c.code_label ? `${_t("në")} ${escapeHtml(c.code_label)}` : _t("në asnjë kod")}</span>
            </li>`;
          }
          if (c.status === "repealed") {
            ...
            return `<li class="cit-row cit-row-warn">
              <span class="cit-status">⚠</span>
              <code>${escapeHtml(c.raw)}</code>
              <span class="cit-meta">${_t("ekziston te")} ${escapeHtml(c.code_label || "")} ${_t("por është SHFUQIZUAR")}${rhead}</span>
            </li>`;
          }
          // needs_code
          ...
          return `<li class="cit-row cit-row-warn">
            <span class="cit-status">?</span>
            <code>${escapeHtml(c.raw)}</code>
            <span class="cit-meta">${_t("kod i pa-specifikuar")} — ${cands ? `${_t("mund të jetë:")} ${cands}` : _t("asnjë përputhje")}</span>
          </li>`;
        }).join("")}
      </ul>
      ${decHtml}
  ...
  function renderTimeline(tl) {
    ...
    const headerTone = mostUrgent
      ? (mostUrgent.urgency === "expired"
          ? (_CAL_IT ? "⛔ TERMINE SCADUTO" : "⛔ AFAT I KALUAR")
          : (_CAL_IT ? "🚨 TERMINE URGENTE" : "🚨 AFAT URGJENT"))
      : (_CAL_IT ? "⏰ Cronologia & scadenze" : "⏰ Kronologjia & afatet");

    const anchorItems = anchors.map((a) => `
      <li class="tl-anchor">
        <span class="tl-dot">●</span>
        <span class="tl-date">${escapeHtml(a.date || (_CAL_IT ? "data sconosciuta" : "data e panjohur"))}</span>
    ...
    const deadlineItems = deadlines.map((d) => {
      ...
      if (d.due_date && days !== null && days !== undefined) {
        ...
        else if (days === 0) timing = _CAL_IT ? `OGGI (${d.due_date})` : `SOT (${d.due_date})`;
        else timing = _CAL_IT ? `tra ${days} giorni (${d.due_date})` : `për ${days} ditë (${d.due_date})`;
      } else if (d.days_after) {
        timing = _CAL_IT
          ? `entro ${d.days_after} giorni da "${escapeHtml(d.anchor_event || "?")}"`
          : `brenda ${d.days_after} ditësh nga "${escapeHtml(d.anchor_event || "?")}"`;
      } else {
        timing = _CAL_IT ? "termine legato a un evento sconosciuto" : "afat i lidhur me një ngjarje të panjohur";
      }
      return `
    ...
    wrap.innerHTML = `
      ...
      ${anchors.length ? `<div class="tl-section-label">${_CAL_IT ? "Eventi che avviano i termini" : "Ngjarjet që nisin afatet"}</div>
        <ul class="tl-anchors">${anchorItems}</ul>` : ""}
      ${deadlines.length ? `<div class="tl-section-label">${_CAL_IT ? "Termini da rispettare" : "Afatet që duhen respektuar"}</div>
  ...
  // Legge TUTTI i file scelti e li accoda al testo, in ordine, con il nome
  // del file come intestazione quando sono piu' di uno. Imposta da se'
  // `multiple`: il markup dei singoli strumenti non va toccato.
  function _readFilesInto(input, ta, statusEl, runBtn) {
    if (!input || !ta) return;
    input.multiple = true;
    input.onchange = async function () {
      ...
      for (var i = 0; i < list.length; i++) {
        try {
          if (statusEl && list.length > 1) {
            statusEl.textContent = TT("Po lexoj dokumentet\u2026") + " " + (i + 1) + "/" + list.length;
          }
          var dd = await _extractFileText(list[i], list.length > 1 ? null : statusEl);
          var tx = (dd.text || "").trim();
          if (!tx) continue;
          var head = list.length > 1 ? ("\u2500\u2500 " + list[i].name + " \u2500\u2500\n") : "";
          ta.value = ta.value.trim() ? (ta.value.trim() + "\n\n" + head + tx) : (head + tx);
          letti++;
        } catch (e) {
          if (statusEl) statusEl.textContent = (_CAL_IT ? "Errore: " : "Gabim: ") + e.message;
        }
      }
      if (statusEl) {
        if (letti > 1) statusEl.textContent = "\u2713 " + letti + " " + TT("dokumente u lexuan");
        else if (letti === 1) statusEl.textContent = TT("\u2713 Dokumenti u lexua");
        else statusEl.textContent = TT("Dokumenti nuk ka tekst t\u00eb lexuesh\u00ebm.");
      }
  ...
  function _openTetramorphTool(cfg) {
    var ov = document.getElementById(cfg.id);
    if (ov) ov.remove();
    ov = document.createElement("div");
    ov.id = cfg.id; ov.className = "ac-overlay";
    var langHtml = cfg.langBar
      ? '<div class="pk-langs">' + ["sq", "it", "en"].map(function (l) {
          return '<button type="button" class="pk-lang' + (l === (cfg.langDefault || "it") ? " on" : "") + '" data-lang="' + l + '">' + l.toUpperCase() + '</button>';
        }).join("") + '<span class="pk-langs-hint">' + t('Gjuha e synuar') + '</span></div>'
      : "";
    ...
    ov.querySelector(".ac-x").onclick = close;
    ov.addEventListener("click", function (e) { if (e.target === ov) close(); });
    Array.prototype.forEach.call(ov.querySelectorAll(".pk-lang"), function (b) {
      b.onclick = function () {
        Array.prototype.forEach.call(ov.querySelectorAll(".pk-lang"), function (x) { x.classList.remove("on"); });
        b.classList.add("on");
      };
    });
    _readFilesInto(ftFile, ta, status, run);
    run.onclick = async function () {
      ...
      try {
        ...
        if (cfg.langBar) {
          var _lp = ov.querySelector(".pk-lang.on");
          payload.target = _lp ? _lp.getAttribute("data-lang") : (cfg.langDefault || "it");
        }
  ...
  function openAdversary() {
    _openTetramorphTool({ id: "adv-ov", title: "\u2694\ufe0f Kundërshtari",
  ...
  // ── Letra dhe shkresa: shkresa gati për dërgim, e rrënjosur në fashikull ──
  // Katalogu vjen nga serveri sipas juridiksionit të sesionit, prandaj etiketat
  // nuk kanë nevojë për përkthim këtu.
  var _LETTER_KINDS = null;

  async function openLetters() {
    var ov = document.getElementById("letters-ov");
    if (ov) ov.remove();
    ov = document.createElement("div");
    ov.id = "letters-ov"; ov.className = "ac-overlay";
    ov.innerHTML =
      '<div class="ac-modal">' +
        '<div class="ac-head"><span>✉️ ' + t("Letra dhe shkresa") + '</span><button class="ac-x" type="button" aria-label="Mbyll">×</button></div>' +
        '<div class="ac-sub">' + t("Zgjidh kujt i shkruhet. Shkresa ndërtohet mbi fashikullin e hapur dhe mbi nenet e marra nga korpusi — gati për dërgim.") + '</div>' +
        '<select class="lt-kind"><option>' + t("Po ngarkohet…") + '</option></select>' +
        '<div class="lt-info"></div>' +
        '<select class="lt-form">' +
          '<option value="letter">📄 ' + t("Letër zyrtare") + '</option>' +
          '<option value="email">📧 ' + t("Email / PEC") + '</option>' +
        '</select>' +
        '<div class="ac-attach-row"><label class="ac-attach">' +
          t("📎 Bashkëngjit dokumentin e marrë") +
          '<input type="file" class="lt-file" multiple accept=".pdf,.docx,.doc,.txt,.rtf,.jpg,.jpeg,.png,.webp,.tif,.tiff" hidden></label>' +
          '<span class="ac-attach-hint">' + t("letra e pushimit, akti, njoftimi — që t\'i përgjigjemi pikë për pikë") + '</span>' +
        '</div>' +
        '<div class="lt-files"></div>' +
        '<textarea class="ac-ta" placeholder="' + t("Fakte shtesë ose udhëzime: emrat, shumat, datat, çfarë të theksohet. Nëse fashikulli është i plotë, mund ta lësh bosh.") + '"></textarea>' +
        '<div class="ac-row"><button class="ac-run" type="button">' + t("Harto shkresën →") + '</button><span class="ac-status"></span></div>' +
        '<div class="ac-result"></div>' +
      "</div>";
    document.body.appendChild(ov);

    var sel = ov.querySelector(".lt-kind"), info = ov.querySelector(".lt-info"),
        form = ov.querySelector(".lt-form"), ta = ov.querySelector(".ac-ta"),
        run = ov.querySelector(".ac-run"), status = ov.querySelector(".ac-status"),
        result = ov.querySelector(".ac-result");
    var filesBox = ov.querySelector(".lt-files"), fileInput = ov.querySelector(".lt-file");
    // testo dei documenti allegati: tenuto SEPARATO dalle istruzioni
    // dell'avvocato, cosi' il cervello lo tratta come prova da ribattere
    var received = [];

    function paintFiles() {
      filesBox.innerHTML = received.map(function (f, i) {
        return '<span class="lt-chip">📄 ' + escapeHtml(f.name) +
               ' <button type="button" data-i="' + i + '" aria-label="Hiq">×</button></span>';
      }).join("");
      Array.prototype.forEach.call(filesBox.querySelectorAll("button"), function (b) {
        b.onclick = function () { received.splice(+b.dataset.i, 1); paintFiles(); };
      });
    }
    ...
    if (fileInput) fileInput.onchange = async function () {
      ...
      for (var i = 0; i < list.length; i++) {
        ...
        try {
          ...
          var tx = (dd.text || "").trim();
          if (!tx) { status.textContent = t("Dokumenti nuk ka tekst të lexueshëm."); continue; }
          received.push({ name: file.name, text: tx });
          status.textContent = dd.used_vision_ocr ? t("✓ Lexuar me OCR") : t("✓ Dokumenti u lexua");
        } catch (e) {
          status.textContent = (_CAL_IT ? "Errore: " : "Gabim: ") + e.message;
        }
      }
      paintFiles();
      run.disabled = false; fileInput.value = "";
    };
    ...
    function paintInfo() {
      var k = _LETTER_KINDS && _LETTER_KINDS.filter(function (x) { return x.key === sel.value; })[0];
      if (!k) { info.innerHTML = ""; return; }
      var warn = k.family === "claim"
        ? t("Kjo shkresë i drejtohet palës kundërshtare. Forca vjen nga nenet, jo nga kërcënimet: njoftimi se do t'i drejtohemi gjykatës lejohet, kërcënimi me kallëzim penal apo me njoftim te tatimet për të marrë pagesë është shantazh.")
        : (k.family === "report"
            ? t("Kjo shkresë i drejtohet autoritetit. Toni është faktik dhe nuk lidhet me asnjë kërkesë pagese — përndryshe shndërrohet në shantazh.")
            : "");
      info.innerHTML =
        '<div class="lt-meta"><b>' + t("Marrësi") + ':</b> ' + escapeHtml(k.recipient) + '<br>' +
        '<b>' + t("Kanali") + ':</b> ' + escapeHtml(k.channel) + '</div>' +
        (k.note ? '<div class="lt-note">ℹ️ ' + escapeHtml(k.note) + '</div>' : "") +
        (warn ? '<div class="lt-warn">⚠️ ' + escapeHtml(warn) + '</div>' : "");
    }
    sel.onchange = paintInfo;

    try {
      if (!_LETTER_KINDS) {
        var rk = await fetch("/api/letters/kinds");
        var dk = await rk.json();
        _LETTER_KINDS = dk.kinds || [];
      }
      var groups = [], byGroup = {};
      _LETTER_KINDS.forEach(function (k) {
        if (!byGroup[k.group]) { byGroup[k.group] = []; groups.push(k.group); }
        byGroup[k.group].push(k);
      });
      sel.innerHTML = groups.map(function (g) {
        return '<optgroup label="' + escapeHtml(g) + '">' + byGroup[g].map(function (k) {
          return '<option value="' + escapeHtml(k.key) + '">' + escapeHtml(k.emoji + " " + k.label) + '</option>';
        }).join("") + '</optgroup>';
      }).join("");
      paintInfo();
    } catch (e) {
      sel.innerHTML = '<option>' + t("Nuk u ngarkua katalogu") + '</option>';
    }

    run.onclick = async function () {
      var extra = (ta.value || "").trim();
      var receivedText = received.map(function (f) {
        return "── " + f.name + " ──\n" + f.text;
      }).join("\n\n");
      if (!activeCaseId && extra.length < 15 && !receivedText) {
        status.textContent = t("Hap një fashikull ose shkruaj faktet këtu.");
        return;
      }
      run.disabled = true; status.textContent = t("Po hartohet shkresa…"); result.innerHTML = "";
      try {
        var r = await fetch("/api/letters/draft", {
          method: "POST", headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ kind: sel.value, form: form.value,
                                 facts: extra, extra: extra,
        ...
        copy.className = "fd-copy"; copy.type = "button";
        copy.textContent = "📋 " + t("Kopjo letrën");
        copy.onclick = function () {
          // la lettera sola: e' cio' che si incolla nella PEC
          navigator.clipboard.writeText(d.document || d.markdown || "").then(function () {
            copy.textContent = "✓ " + t("U kopjua");
          }).catch(function () {});
        };
        bar.appendChild(copy);

        var dl = document.createElement("button");
        dl.className = "fd-copy"; dl.type = "button";
        dl.textContent = "⬇️ " + t("Shkarko .docx");
        ...
        // PDF: nel server non c'e' alcun generatore e aggiungerne uno
        // significherebbe gestire i font per ë/ç/à. La stampa del browser
        // produce un PDF vero, con i caratteri giusti e zero dipendenze.
        var pdf = document.createElement("button");
        pdf.className = "fd-copy"; pdf.type = "button";
        pdf.textContent = "🖨️ " + t("PDF (shtyp)");
        pdf.onclick = function () {
          var w = window.open("", "_blank");
          if (!w) { status.textContent = t("Blloko dritaret u aktivizua — lejo dritaret."); return; }
          var body = renderMarkdown(d.document || d.markdown || "");
          w.document.write(
            '<!doctype html><html><head><meta charset="utf-8"><title>' +
            escapeHtml(d.label || "Shkresë") + '</title><style>' +
            'body{font:12pt/1.55 Georgia,"Times New Roman",serif;color:#111;' +
            'max-width:19cm;margin:2cm auto;padding:0 1cm}' +
            'h1,h2,h3{font-size:1.05em;margin:1.2em 0 .4em}' +
            ...
            '@page{margin:2cm}</style></head><body>' + body + '</body></html>');
          w.document.close();
          setTimeout(function () { w.focus(); w.print(); }, 350);
        };
        bar.appendChild(pdf);
        result.appendChild(bar);
        _addSaveToCase(result, "letter", (d.label || "Shkresë"), d.markdown || "");
      } catch (e) {
        status.textContent = (_CAL_IT ? "Errore: " : "Gabim: ") + e.message;
      } finally { run.disabled = false; }
    };
  ...
  function openTetramorphDraft() {
    ...
    _readFilesInto(fdFile, ta, status, run);
    function close() { ov.remove(); }
    ov.querySelector(".ac-x").onclick = close;
    ov.addEventListener("click", function (e) { if (e.target === ov) close(); });
    run.onclick = async function () {
      ...
      try {
        var r = await fetch("/api/fable-draft", {
          method: "POST", headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ kind: kind.value, brief: brief, case_id: activeCaseId || "", use_clauses: !!(ov.querySelector(".fd-useclauses") && ov.querySelector(".fd-useclauses").checked) }),
        });
        var d = await r.json();
        if (!r.ok || d.error) throw new Error(d.error || ("HTTP " + r.status));
        status.textContent = "";
  ...
  async function openExpertise(preselect) {
    ...
    function detail(t) {
      ...
      _readFilesInto(file, ta, status, run);
      run.onclick = async function () {
        ...
        try {
          var r = await fetch("/api/expertise/analyze", { method: "POST", headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ case_type: t.key, facts: facts, case_id: activeCaseId || "" }) });
          var d = await r.json();
          if (!r.ok || d.error) throw new Error(d.error || ("HTTP " + r.status));
          status.textContent = "";
          result.innerHTML = '<div class="fd-out"></div>';
  ...
  function openProsecutor() {
    _openTetramorphTool({ id: "pros-ov", title: "\ud83c\udfdb\ufe0f Analiza e prokurorit",
    ...
    run.onclick = async function () {
      ...
      try {
        var r = await fetch("/api/notary/draft", { method: "POST", headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ deed_type: kind.value, details: details, use_clauses: !!(ov.querySelector(".nd-useclauses") && ov.querySelector(".nd-useclauses").checked), case_id: activeCaseId || "" }) });
        var d = await r.json();
        if (!r.ok || d.error) throw new Error(d.error || ("HTTP " + r.status));
        status.textContent = "";
        result.innerHTML = '<div class="fd-out"></div>';
  ...
  function openNotaryCheck() {
    _openTetramorphTool({ id: "ncheck-ov", title: "\u2705 Kontroll vlefshmërie akti",
  ...
  function openNotarySuccession() {
    _openTetramorphTool({ id: "nsucc-ov", title: "\u2696\ufe0f Analizë trashëgimie",
      ...
      endpoint: "/api/notary/succession", payloadKey: "situation", attach: false, attach: true, source: "notary", saveTitle: "Analizë trashëgimie" });
  }
  ...
  function openDocsChecklist() {
    _openTetramorphTool({ id: "ndocs-ov", title: "📋 Dokumentet e nevojshme",
      ...
      endpoint: "/api/notary/documents", payloadKey: "act", attach: false, attach: true, source: "notary", saveTitle: "Dokumentet e nevojshme" });
  }

  function openRevocation() {
    _openTetramorphTool({ id: "nrev-ov", title: "♻️ Revokim prokure",
  ...
  function openProsPlan() {
    _openTetramorphTool({ id: "pplan-ov", title: "🔎 Plani i hetimit",
  ...
  function openProsMeasure() {
    _openTetramorphTool({ id: "pmeas-ov", title: "🔒 Kërkesë për masë sigurimi",
  ...
  function openProsDismissal() {
    _openTetramorphTool({ id: "pdism-ov", title: "🗄️ Kërkesë për pushim / mosfillim",
  ...
  function openProsStress() {
    _openTetramorphTool({ id: "pstress-ov", title: "🛡️ Stres-test i aktit (ana e mbrojtjes)",
  ...
  function openProsComplaint() {
    _openTetramorphTool({ id: "pcompl-ov", title: "🧾 Ndihmë për kallëzim penal",
  ...
  function openProsVictim() {
    _openTetramorphTool({ id: "pvict-ov", title: "⚖️ Të drejtat e viktimës",
      ...
      endpoint: "/api/prosecutor/victim-rights", payloadKey: "facts", attach: false, attach: true, source: "prosecutor", saveTitle: "Të drejtat e viktimës" });
  }

  function openProsAppeal() {
    _openTetramorphTool({ id: "pappeal-ov", title: "📤 Ankim kundër pushimit",
  ...
  function openProsDelay() {
    _openTetramorphTool({ id: "pdelay-ov", title: "📨 Ankesa për vonesa",
      ...
      endpoint: "/api/prosecutor/delay", payloadKey: "facts", attach: false, attach: true, source: "prosecutor", saveTitle: "Ankesa për vonesa" });
  }
    ...
    run.onclick = async function () {
      ...
      try {
        var r = await fetch("/api/prosecutor/investigative-act", { method: "POST", headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ kind: kind.value, facts: facts, case_id: activeCaseId || "" }) });
        var d = await r.json();
        if (!r.ok || d.error) throw new Error(d.error || ("HTTP " + r.status));
        status.textContent = "";
        result.innerHTML = '<div class="fd-out"></div>';
  ...
  function openDeepVerify() {
    _openTetramorphTool({ id: "dverify-ov", title: "🔬 Verifikim i thellë",
  ...
  function openLawLive() {
    _openTetramorphTool({ id: "lawlive-ov", title: "🌐 A është ende në fuqi?",
      ...
      endpoint: "/api/law-live", payloadKey: "query", attach: false, attach: true, source: "research", saveTitle: "Kontroll ligji (live)" });
  }
  ...
  async function openVideo() {
    ...
    ov.innerHTML = '<div class="ac-modal exp-modal">' +
      '<div class="ac-head"><span data-i18n="vd_title">🎥 Video dhe audio</span>' +
        '<button class="ac-x" type="button" aria-label="Mbyll">×</button></div>' +
      '<div class="exp-body">' +
        '<div class="exp-sub" data-i18n="vd_sub">Provat video dhe audio: të dhënat e skedarit, vërejtjet mbi origjinalitetin, rrjedha kohore me minutazhe dhe fjalët e transkriptuara. Pastaj krahasimi me procesverbalet — aty ku nuk përputhen, aty është puna.</div>' +
        '<div class="vd-note" data-i18n="vd_note">⚠️ Modeli nuk e sheh videon: sheh fotograme të nxjerra prej saj — momenti vendimtar mund të bjerë midis dy fotogramave. Transkripti i audios është bozë (në shqip vetëm për orientim). Nuk identifikon persona dhe nuk vendos për fajësinë.</div>' +
        '<details class="fk-sec" open><summary data-i18n="vd_s_up">📼 Video dhe audio të fashikullit</summary>' +
          '<div class="ac-row">' +
            '<label class="fk-upload" data-i18n="vd_b_up">📎 Ngarko video ose audio' +
              '<input type="file" multiple class="vd-file" accept=".mp4,.mov,.avi,.mkv,.m4v,.webm,.mpg,.wmv,.dav,video/*,.mp3,.wav,.m4a,.aac,.ogg,.opus,.amr,audio/*" hidden></label>' +
            '<span class="ac-status vd-up-st"></span>' +
          '</div>' +
          '<div class="vd-list"></div></details>' +
        '<details class="fk-sec"><summary data-i18n="vd_s_cmp">⚖️ Krahaso me fashikullin</summary>' +
    ...
    async function ricarica() {
      if (!activeCaseId) {
        lista.innerHTML = '<div class="fk-warn">' + TT("Hap një rast për të ngarkuar video ose audio.") + '</div>';
        return;
      }
      ...
      var vs = j.videos || [];
      if (!vs.length) {
        lista.innerHTML = '<div class="fk-warn">' + TT("Asnjë provë multimediale ende. Ngarko video (mp4, mov, dav…) ose audio (mp3, m4a, wav…).") + '</div>';
        return;
      }
    ...
    inp.onchange = async function () {
      ...
      upSt.textContent = TT("⏳ Po ngarkoj… (skedarët janë të mëdhenj, mund të zgjasë)");
    ...
    cmpBtn.onclick = async function () {
      ...
      cmpSt.textContent = TT("⏳ Po krahasoj provat me dokumentet…");
      cmpRes.innerHTML = "";
      try {
  ...
  async function openFascikull() {
    ...
    async function refreshInfo() {
      ...
      info.innerHTML = '<div class="fk-uprow"><label class="fk-upload">' + t('📎 Ngarko dokumente') + '<input type="file" multiple class="fk-file" accept=".pdf,.jpg,.jpeg,.png,.webp,.tif,.tiff,.docx,.txt,.mp4,.mov,.avi,.mkv,.m4v,.webm,.mpg,.wmv,.dav,video/*,.mp3,.wav,.m4a,.aac,.ogg,.opus,.amr,audio/*" hidden></label> ' + txt + '</div>';
    ...
    _readFilesInto(ispFile, ispTa, ispSt, ispBtn);
    ...
    run.onclick = async function () {
      ...
      try {
        var r = await fetch("/api/afati/compute", { method: "POST", headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ trigger: trig.value, event_date: date.value || "", facts: (ta.value || "").trim(), case_id: activeCaseId || "" }) });
        var d = await r.json();
        if (!r.ok || d.error) throw new Error(d.error || ("HTTP " + r.status));
        status.textContent = "";
        result.innerHTML = '<div class="fd-out"></div>';
  ...
  async function openVerifySubject() {
    ...
    ov.innerHTML = '<div class="ac-modal">' +
      ...
      '<div class="ac-sub">' + ((document.body.dataset.lang === "it")
        ? "Incolla la visura camerale (Registro Imprese) o i dati del soggetto: si analizzano stato, poteri degli amministratori, soci e rischi (liquidazione/cancellazione, red flag). Ausilio — per l'atto, conferma sul portale ufficiale."
        : t("Kërko subjektin drejtpërdrejt në QKB (butoni «Kërko»), ose ngjit ekstraktin/rezultatin manualisht nga qkb.gov.al. Analizohet statusi, administratorët e tagrat, ortakët dhe rreziqet (likuidim/çregjistrim, red-flag). Ndihmesë — për aktin, konfirmo te portali zyrtar.")) + '</div>' +
      // QKB e' il registro ALBANESE: in sessione italiana la riga live non ha senso (v9.313)
      '<div class="qkb-search" style="' + ((document.body.dataset.lang === "it") ? "display:none;" : "") + 'background:rgba(201,162,39,.07);border:1px solid var(--line,#d9cfc0);border-radius:8px;padding:10px;margin-bottom:10px">' +
  ...
  async function openVerifyProperty() {
    ...
    if (ov) ov.remove();
    ov = document.createElement("div");
    ov.id = "vprop-ov"; ov.className = "ac-overlay";
    ov.innerHTML = '<div class="ac-modal">' +
      '<div class="ac-head"><span>' + tMode("🧾 Verifiko pronësinë") + '</span><button class="ac-x" type="button" aria-label="Mbyll">×</button></div>' +
      '<div class="ac-sub">' + t("Ngarko certifikatën/kartelën e pronësisë (ASHK) ose ekstraktin (QKB) dhe përshkruaj veprimin. Kontrollon pronarin e regjistruar, identifikimin e pasurisë dhe barrët (hipotekë/sekuestro/servitut) e i kryqëzon me veprimin. NDIHMESË: certifikatën zyrtare e merr vetë noteri — nuk lidhet live me ASHK.") + '</div>' +
      '<input type="text" class="vprop-tx" style="width:100%;box-sizing:border-box;padding:9px 12px;border:1px solid var(--line,#d9cfc0);border-radius:8px;font-size:14px;margin-bottom:8px" placeholder="' + t("Veprimi: p.sh. «shitje e apart. 7/512 nga Arben Hoxha te Blerina Leka»") + '" />' +
      '<div class="ac-attach-row"><label class="ac-attach">' + t("📎 Bashkëngjit certifikatën (PDF/foto)") + '<input type="file" class="vprop-file" accept=".pdf,.jpg,.jpeg,.png,.webp,.tif,.tiff,.docx" hidden multiple></label></div>' +
      '<textarea class="ac-ta vprop-cert" placeholder="' + t("Ose ngjit këtu tekstin e certifikatës/kartelës…") + '"></textarea>' +
      '<div class="ac-row"><button class="ac-run" type="button">' + t("Verifiko →") + '</button><span class="ac-status"></span></div>' +
      '<div class="ac-result"></div>' +
      "</div>";
    document.body.appendChild(ov);
    applyStaticI18n(ov);
    var tx = ov.querySelector(".vprop-tx"), cert = ov.querySelector(".vprop-cert"),
        run = ov.querySelector(".ac-run"), status = ov.querySelector(".ac-status"),
        result = ov.querySelector(".ac-result"), file = ov.querySelector(".vprop-file");
    function close() { ov.remove(); }
    ov.querySelector(".ac-x").onclick = close;
    ov.addEventListener("click", function (e) { if (e.target === ov) close(); });
    if (file) file.onchange = async function () {
      ...
      for (var i = 0; i < files.length; i++) {
        try { var dd = await _extractFileText(files[i], status); var t2 = (dd.text || "").trim();
          if (t2) cert.value = cert.value.trim() ? (cert.value.trim() + "\n\n" + t2) : t2; }
        catch (e) { status.textContent = (_CAL_IT ? "Errore: " : "Gabim: ") + e.message; }
      }
      status.textContent = "✓ " + files.length + t(" dokument(e) u lexuan"); run.disabled = false; file.value = "";
    };
    run.onclick = async function () {
      var certText = (cert.value || "").trim();
      if (certText.length < 40) { status.textContent = t("Ngarko ose ngjit certifikatën e pronësisë."); return; }
      run.disabled = true; status.textContent = t("Po verifikoj pronësinë dhe barrët… (~1-2 min)"); result.innerHTML = "";
      try {
        var r = await fetch("/api/notary/verify-property", { method: "POST", headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ certificate: certText, transaction: (tx.value || "").trim() }) });
        var d = await r.json();
        if (!r.ok || d.error) throw new Error(d.error || ("HTTP " + r.status));
        status.textContent = "";
        result.innerHTML = '<div class="fd-out"></div>';
        ...
        copy.onclick = function () { navigator.clipboard.writeText(d.markdown || "").then(function () { copy.textContent = "✓ U kopjua"; }).catch(function () {}); };
        result.appendChild(copy);
        _addSaveToCase(result, "notary", "Verifikim pronësie", d.markdown || "");
      } catch (e) { status.textContent = (_CAL_IT ? "Errore: " : "Gabim: ") + e.message; }
      finally { run.disabled = false; }
    };
    setTimeout(function () { try { tx.focus(); } catch (e) {} }, 50);
  }
    ...
    run.onclick = async function () {
      ...
      try {
        ...
        go.onclick = function () {
          ...
          var fn = mode === "prokuror" ? openIndictment : (mode === "avokat" ? openTetramorphDraft : openNotaryDeed);
          close();
          setTimeout(function () {
            fn();
            setTimeout(function () {
  ...
  async function openWhatIf() {
    ...
    ov.querySelector(".ac-x").onclick = close;
    ov.addEventListener("click", function (e) { if (e.target === ov) close(); });
    _readFilesInto(file, actTa, status, run);
    run.onclick = async function () {
  ...
  var I18N_IT = {
    ...
    vd_sub: "Le prove video e audio: dati del file, rilievi sull'originalità, linea temporale con i minutaggi e le parole trascritte. Poi il confronto con i verbali — dove non tornano, lì c'è il lavoro.",
    vd_note: "⚠️ Il modello non guarda il video: guarda fotogrammi estratti da esso — l'istante decisivo può cadere fra due fotogrammi. La trascrizione dell'audio è una bozza (in albanese solo per orientarsi). Non identifica persone e non decide sulla colpevolezza.",
    vd_s_up: "📼 Video e audio del fascicolo",
    vd_b_up: "📎 Carica video o audio",
    vd_s_cmp: "⚖️ Confronta col fascicolo",
    ...
    it_03: "Post-mortem strutturato del fascicolo + lezioni emerse dai casi simili precedenti",
    it_04: "Per ogni tipo di causa: base giuridica, elementi, prove, termini, difese — e le due menti (pubblico ministero + avvocato)",
    it_05: "— I TERMINI —",
    it_06: "Motore delle scadenze",
    it_07: "Da un evento iniziale → tutti i termini procedurali (fondati) → nel calendario con gli avvisi",
    it_08: "— IL FASCICOLO —",
    it_09: "Tutto quello che hai salvato — risposte, atti, procure, lettere — da tutti i casi, in un posto solo",
    it_10: "Fascicolo intelligente",
    it_11: "Il cittadino racconta a voce o per iscritto → se c'è un caso, quanto è urgente, chi lo aiuta, e il primo documento",
    it_12: "Checklist del fascicolo",
    it_13: "Per una causa o un atto: documenti e prove richiesti vs presenti vs mancanti o scaduti + completezza",
    it_14: "Registro (cerca atti)",
    it_15: "Ricerca semantica negli atti salvati dello studio — per significato, soggetto o immobile",
    it_16: "Salva le clausole preferite; il drafter (atto/procura/Redigi) le usa per mantenere il tuo stile",
    it_17: "— LA PROCURA —",
    it_18: "Super Procuratore",
    it_19: "Prescrizione e termini",
    it_20: "Calcola il termine di prescrizione (penale/civile) e se è già scaduto",
    it_21: "Procure, atti, dichiarazioni, controllo di validità, conflitti, successioni, revoche, documenti, tariffe",
    it_22: "Lettere e atti",
    it_23: "Lettere pronte da inviare dal fascicolo — al datore di lavoro, al fisco, alla procura, agli enti",
    it_24: "Chiedi all'Avvocato del Diavolo",
    it_25: "Carica PDF, foto o scansioni dei documenti della causa (decisioni, contratti, atti di citazione, notifiche). Super Avokati li leggerà e li userà come prova per risposte più precise.",
    it_26: "🗂️ Ricerche salvate",
    it_27: "Ancora nessuna ricerca salvata. Clicca “💾 Salva nel fascicolo” su una risposta o su uno strumento PRO.",
    it_28: "🔎 Interroga i documenti del fascicolo",
    it_29: "🔍 Trova l'ago nel fascicolo",
    it_30: "decisione d'appello — i termini per il ricorso",
    it_31: "KYC per un nuovo cliente S.r.l.",
    it_32: "🎙️ Intake cliente con l'AI",
    it_33: "L'AI fa domande passo dopo passo. Alla fine produce una sintesi strutturata e apre il caso da sola.",
    it_34: "Invia la risposta →",
    it_35: "⟵ Fai un'altra domanda",
    it_36: "👤 I clienti del caso",
    it_37: "Crea un link privato (magic-link) con cui il cliente segue l'andamento: la fase, i prossimi incontri, gli aggiornamenti che hai condiviso.",
    it_38: "+ Aggiungi cliente",
    it_39: "Il link viene generato da solo. Puoi mandarlo al cliente per SMS, WhatsApp o email.",
    it_40: "Non ci sono ancora clienti collegati a questo caso.",
    it_41: "📢 Aggiornamenti per il cliente",
    it_42: "Quello che scrivi qui compare nel portale del cliente. Usa «Traduci per il cittadino» per riscrivere un testo tecnico in parole semplici, oppure «Suggerisci aggiornamento» per una proposta.",
    it_43: "Incolla un testo tecnico (citazioni di articoli, decisioni, atti). L'AI restituisce una versione semplice che il cliente capisce.",
    it_44: "Incolla il testo completo del contratto. L'AI restituisce un semaforo 🟢🟡🔴 per ogni clausola, con obblighi, termini e rischi GDPR.",
    it_45: "Registra le ore di lavoro e genera le parcelle. Le tariffe sono indicative secondo la Camera Nazionale degli Avvocati d'Albania.",
    it_46: "Rappresentanza in giudizio",
    it_47: "Incontro con il cliente",
    it_48: "⚖️ Avvocato",
    it_49: "🏛️ Procuratore",
    it_50: "Quanti casi, quante udienze e quante scadenze ha ciascun membro — per decidere chi può prendere il caso nuovo.",
    it_51: "Prima di accettare un nuovo cliente, cerca il nome della parte (cliente o controparte) nello storico dello studio.",
    it_52: "BM25 su 1.258 decisioni (Costituzionale · Cassazione · CEDU-Albania) + ratio AI → le mosse che hanno vinto, le trappole che hanno perso, il kill-shot.",
    it_53: "Le 3 lezioni più utili dai casi precedenti più simili al fascicolo attuale.",
    it_54: "Ogni atto nuovo (legge, decisione, direttiva) che carichi qui viene classificato dall'AI e confrontato con i tuoi fascicoli aperti. Risultato: un avviso diretto su cosa cambia per te.",
    it_55: "⬆️ Carica una modifica",
    it_56: "Nessun avviso. Carica una modifica normativa o aspetta la prossima decisione.",
    it_57: "Incolla il testo completo dell'atto (legge, delibera, direttiva, decisione). L'AI lo classifica e lo confronta con i tuoi fascicoli aperti.",
    it_58: "Corte di Cassazione",
    it_59: "Corte Costituzionale",
    it_60: "Memorandum calibrato sul giudice specifico: inquadramento della causa, P(vittoria), le tue debolezze che verranno attaccate, i punti forti dell'avversario che il giudice accetterà.",
    it_61: "Corte di Cassazione",
    it_62: "Corte d'Appello",
    it_63: "Tribunale di primo grado",
    it_64: "Corte Costituzionale",
    it_65: "Tribunale amministrativo",
    it_66: "Tribunale delle imprese (Tirana)",
    it_67: "Carica il documento della società → l'AI estrae soci, CdA, procuratori e scadenze. Controlla il firmatario prima di firmare. Genera la checklist KYC/AML (Legge 9917/2008).",
    it_68: "Procura / Autorizzazione",
    it_69: "Caricamento…",
    it_70: "Caricamento…",
    it_71: "Caricamento…",
  ...
  var T_IT = {
    ...
    "⏳ Po ngarkoj… (skedarët janë të mëdhenj, mund të zgjasë)": "⏳ Caricamento… (i file sono grandi, può volerci un po')",
    "Ngarkuar. Analiza vazhdon në sfond.": "Caricato. L'analisi continua in sottofondo.",
    "⏳ Po krahasoj provat me dokumentet…": "⏳ Confronto le prove con i documenti…",
    "Skedar shumë i madh (max 25 MB dokument / 200 MB audio / 500 MB video): ": "File troppo grande (max 25 MB documento / 200 MB audio / 500 MB video): ",
  ...
  // Registrazioni depositate come prova: telefonate, vocali, audio di
  // telecamera. Stessi formati di config.AUDIO_EXTENSIONS.
  var AUDIO_EXT = /\.(mp3|wav|m4a|aac|ogg|oga|opus|flac|wma|amr|3ga|caf|aiff|aif)$/i;
  ...
  Object.assign(T_IT, {
    "🧾 Verifiko pronësinë": "🧾 Verifica proprietà",
    "Verifiko pronësinë & barrët": "Verifica proprietà & gravami",
    "Ngarko certifikatën/kartelën e pronësisë (ASHK) ose ekstraktin (QKB) dhe përshkruaj veprimin. Kontrollon pronarin e regjistruar, identifikimin e pasurisë dhe barrët (hipotekë/sekuestro/servitut) e i kryqëzon me veprimin. NDIHMESË: certifikatën zyrtare e merr vetë noteri — nuk lidhet live me ASHK.": "Carica la certificata/scheda di proprietà (ASHK) o la visura (QKB) e descrivi l'operazione. Controlla il proprietario registrato, l'identificazione dell'immobile e i gravami (ipoteca/sequestro/servitù) e li incrocia con l'operazione. ASSISTIVO: la certificata ufficiale la richiede il notaio — non si collega in tempo reale ad ASHK.",
    "Veprimi: p.sh. «shitje e apart. 7/512 nga Arben Hoxha te Blerina Leka»": "L'operazione: es. «vendita dell'appart. 7/512 da Arben Hoxha a Blerina Leka»",
    "📎 Bashkëngjit certifikatën (PDF/foto)": "📎 Allega la certificata (PDF/foto)",
    "Ose ngjit këtu tekstin e certifikatës/kartelës…": "Oppure incolla qui il testo della certificata/scheda…",
    "Verifiko →": "Verifica →",
  ...
  // v9.313 — SESSIONE ITALIANA = SOLO ITALIANO (regola ferrea del titolare, 14 set):
  // i 15 residui trovati dal DOM vivo in sessione IT — tooltip senza data-i18n-title,
  // l'area upload, le opzioni del profilo dello studio. Aggancio per testo esatto.
  Object.assign(T_IT, {
    "Tërhiqi këtu ose kliko për të zgjedhur · PDF, Word, foto · video dhe audio · max 25 MB dokument / 200 MB audio / 500 MB video": "Trascina qui o clicca per scegliere · PDF, Word, foto · video e audio · max 25 MB documento / 200 MB audio / 500 MB video",
    "Gjithçka e ruajtur dhe e ngarkuar, nga të gjitha rastet": "Tutto ciò che hai salvato e caricato, da tutti i casi",
    "Merr njoftim kur analiza mbaron, edhe nëse mbyll faqen": "Ricevi una notifica quando l'analisi termina, anche se chiudi la pagina",
    "Rilexo kushtet, privatësinë dhe marrëveshjen për të dhënat": "Rileggi le condizioni, la privacy e l'accordo sui dati",
    "Bashkëngjit PDF, foto, docx": "Allega PDF, foto, docx",
    "Konteksti total i përpunuar (hyrje + cache). Kjo është pesha e vërtetë.": "Contesto totale elaborato (input + cache). Questo è il peso reale.",
    "Pesha e konsumit, e llogaritur nga ofruesi. Me abonim nuk paguhet për thirrje: shërben për të krahasuar.": "Peso del consumo, calcolato dal fornitore. Con l'abbonamento non si paga a chiamata: serve per confrontare.",
    "Sa përqind e konsumit të periudhës i takon këtij studioje": "Quale percentuale del consumo del periodo spetta a questo studio",
    "Tavani javor i konsumit për këtë studio ($ peshë). Bosh = pa mbikëqyrje.": "Tetto settimanale di consumo per questo studio ($ peso). Vuoto = senza controllo.",
    "— gjuha e akteve…": "— lingua degli atti…", "Shqip": "Albanese", "Italisht": "Italiano", "Të dyja": "Entrambe",
    "stili…": "stile…", "Formal": "Formale", "I përmbledhur": "Sintetico", "I detajuar": "Dettagliato"
  });
  // v9.316 — etichette FISSE dei pannelli (allerta, nullità, mappa prove, contraddizioni,
  // distinguishing) che in sessione IT restavano albanesi (screenshot del titolare 14 set).
  Object.assign(T_IT, {
    "Pse:": "Perché:", "⏰ Afati:": "⏰ Termine:", "▶ Veprim sot:": "▶ Azione oggi:", "▶ Veprim:": "▶ Azione:",
    "🔄 BARRA E ZHVENDOSUR": "🔄 ONERE INVERTITO",
    "Ligji e zhvendos barrën e provës mbi palën tjetër": "La legge sposta l'onere della prova sulla controparte",
    "Për çdo pretendim tregojmë ÇFARË duhet provuar dhe KUSH duhet ta provojë. Kur ligji e zhvendos barrën (punë, diskriminim, konsumator, dhunë në familje), pala tjetër është ajo që duhet të provojë të kundërtën.": "Per ogni pretesa indichiamo COSA va provato e CHI deve provarlo. Quando la legge inverte l'onere (lavoro, discriminazione, consumatore, violenza domestica), è la controparte a dover provare il contrario.",
    "Këto janë mospërputhjet mes dokumenteve të dosjes. Një avokat i mirë i përdor si levë — dyshim për besueshmëri, shkak për pavlefshmëri, bazë për negocim.": "Queste sono le contraddizioni tra i documenti del fascicolo. Un buon avvocato le usa come leva — dubbio sull'attendibilità, motivo di nullità, base per la trattativa.",
    "Çdo vendim më poshtë u gjet nga BM25 si i ngjashëm me rastin — dhe për secilin është bërë distinguishing: pse nuk aplikohet, ose si mbrohemi nëse aplikohet.": "Ogni decisione qui sotto è stata trovata da BM25 come simile al caso — e per ciascuna è stato fatto il distinguishing: perché non si applica, o come ci difendiamo se si applica."
  });
  ...
  function initModeBar() {
    ...
    var FN = {
      ...
      openTetramorphDraft: openTetramorphDraft, openDevilConsult: openDevilConsult,
    ...
    var TOOLS = {
      avokat: [["\ud83c\udfaf Modele Ekspertize", "openExpertise"], ["\ud83d\uddc2\ufe0f Fashikulli", "openFascikull"], ["\u23f0 Afatet", "openAfati"], ["\ud83d\udfe2 Ligj i gjall\u00eb", "openLivingHub"], ["\ud83c\udf10 P\u00ebrkthim", "openPerkthim"]],
      prokuror: [["\ud83c\udfdb\ufe0f Super Prokurori", "openProsHub"], ["\ud83d\uddc2\ufe0f Fashikulli", "openFascikull"], ["\u23f0 Afatet", "openAfati"], ["\ud83c\udfaf Modele Ekspertize", "openExpertise"], ["\ud83d\udfe2 Ligj i gjall\u00eb", "openLivingHub"], ["\ud83c\udf10 P\u00ebrkthim", "openPerkthim"]],
      noter: [["\ud83d\udcdc Super Noteri", "openNoterHub"], ["\ud83d\uddc2\ufe0f Fashikulli", "openFascikull"], ["\ud83d\udfe2 Ligj i gjall\u00eb", "openLivingHub"], ["\ud83c\udf10 P\u00ebrkthim", "openPerkthim"]]
    };
  ...
  function openPerkthim() {
    // verso: chi lavora in shqip di solito traduce VERSO l'italiano, e
    // viceversa — il default segue la lingua della UI, poi si cambia a click
    var def = _CAL_IT ? "sq" : "it";
    _openTetramorphTool({ id: "perkthim-ov", title: "\ud83c\udf10 Përkthim ligjor",
      sub: "Ngjit tekstin ose bashkangjit PDF/foto/screenshot — teksti nxirret vetë (edhe nga fotot) dhe gjuha e burimit njihet vetë. Përkthim juridik me terminologji të qëndrueshme dhe rileximin e një juristi të gjuhës së synuar. Dokumenti NUK del jashtë platformës. Nuk zëvendëson përkthyesin e betuar.",
      placeholder: "Ngjit këtu tekstin për t'u përkthyer\u2026",
      btn: "Përkthe \u2192", loading: "Po përkthej\u2026 (dokumentet e gjata duan disa minuta)",
      endpoint: "/api/translate", payloadKey: "text", attach: true,
      langBar: true, langDefault: def,
      source: "perkthim", saveTitle: "Përkthim ligjor" });
  }

  function openIndictment() {
    _openTetramorphTool({ id: "indict-ov", title: "\ud83d\udcdc Aktakuzë (prokuror)",
  ...
  function openPrescription() {
    _openTetramorphTool({ id: "presc-ov", title: "\u23f0 Parashkrimi & afatet",
  ...
  function openActCheck() {
    ...
    _readFilesInto(acFile, ta, status, run);
    setTimeout(function () { ta.focus(); }, 50);
  }
  ...
  proMenu?.querySelectorAll("[data-pro]").forEach((btn) => {
    btn.addEventListener("click", () => {
      ...
      else if (key === "fabledraft") { openTetramorphDraft(); }
  ...
  const genioResumeBtn = document.getElementById("genio-resume");
  const genioDescEl   = document.getElementById("genio-description");
  const genioStatusEl = document.getElementById("genio-status");
  // La graffetta del Genio passa dal gestore unico degli allegati:
  // carica, fa estrarre il testo al server (PDF/foto/docx/doc/txt) e lo
  // mette nella descrizione. Stessi formati e stesso comportamento degli
  // altri strumenti — nessun percorso nuovo.
  _readFilesInto(document.getElementById("genio-file"), genioDescEl,
                 genioStatusEl, genioRunBtn);
  ...
  genioRunBtn?.addEventListener("click", () => {
    ...
    if (genioResumeBtn) genioResumeBtn.hidden = true;
  ...
  // La ripresa: rifà SOLO le menti non riuscite, tiene le buone.
  function _genioResume() {
    if (!activeCaseId || !_genioBriefId) return;
    if (_genioES) { try { _genioES.close(); } catch (e) {} _genioES = null; }
    const foot = document.getElementById("genio-footer"); if (foot) foot.remove();
    genioGrid.querySelectorAll(".genio-card").forEach(c => {
      if (c.classList.contains("is-done")) return;
  ...
  genioResumeBtn?.addEventListener("click", _genioResume);

  // Mostra/nasconde il pulsante di ripresa accanto a «Lësho Genio»: c'è un
  // brief con menti mancanti/fallite, il giro non è in corso, la griglia è su.
  function _genioUpdateResume() {
    if (!genioResumeBtn) return;
    const cards = Array.prototype.slice.call(genioGrid.querySelectorAll(".genio-card"));
    const running = cards.some(c => c.classList.contains("is-running"));
    const badN = cards.filter(c => !c.classList.contains("is-done")).length;
    if (_genioBriefId && badN > 0 && !running && !genioGrid.hidden) {
      genioResumeBtn.textContent = (_CAL_IT ? "\ud83d\udd04 Riprova (" : "\ud83d\udd04 Provo p\u00ebrs\u00ebri (") + badN + ")";
      genioResumeBtn.hidden = false;
    } else {
      genioResumeBtn.hidden = true;
    }
  }

  // Footer sotto il brief: SOLO Ruaj në fashikull / DOCX / PDF.
  function _genioFooter() {
    ...
    if (!cards.some(c => c.classList.contains("is-done"))) return;    // nessuna lente riuscita
    const md = _genioBriefMarkdown();
    if (!md) return;
    ...
    genioGrid.after(f);
    try { _addSaveToCase(f, "genio", _CAL_IT ? "Genio \u2014 brief" : "Genio \u2014 brief", md); } catch (e) {}
  }
  ...
  function renderGenioContent(key, res) {
    ...
    const p = res.parsed;
    if (key === "riframing") {
      return `<div class="gn-section"><span class="gn-label">${t("Aktual:")}</span> ${htmlEsc(p.current_framing||"—")}</div>` +
        `<ul>${(p.alternatives||[]).map(a => `<li><strong>${htmlEsc(a.name)}</strong> (rank ${a.rank}) — ${htmlEsc(a.thesis||"")}</li>`).join("")}</ul>` +
        `<div class="gn-section"><span class="gn-label">${t("Verdikti:")}</span> ${htmlEsc(p.verdict||"")}</div>`;
    }
    if (key === "kill_shot") {
      return `<ul>${(p.kill_shots||[]).map(k => `<li><strong>#${k.rank} ${htmlEsc(k.title||"")}</strong> — ${t("letaliteti")} ${k.lethality}/10. ${htmlEsc(k.mechanics||"")}<br><em>${t("Kontrasulmi:")}</em> ${htmlEsc(k.our_counter_prep||"")}</li>`).join("")}</ul>` +
        (p.fatal_combo_warning ? `<div class="gn-section"><span class="gn-label">Combo:</span> ${htmlEsc(p.fatal_combo_warning)}</div>` : "");
    }
    if (key === "leverage") {
      return `<ul>${(p.leverage_points||[]).map(l => `<li><strong>#${l.rank} ${htmlEsc(l.lever||"")}</strong> (${t("rrezik etik:")} ${l.ethical_risk})<br>${htmlEsc(l.why_it_works||"")}<br><em>${t("Aktivizim:")}</em> ${htmlEsc(l.activation||"")}</li>`).join("")}</ul>`;
    }
    if (key === "decision_tree") {
      return `<div class="gn-section"><span class="gn-label">${t("Rrënja:")}</span> ${htmlEsc(p.root_label||"")}</div>` +
    ...
    if (key === "brutal_truth") {
      ...
      return `<div class="gn-section"><span class="gn-label">${t("Vlera reale:")}</span> ${fmtEurFloat(rv.low)} → <strong>${fmtEurFloat(rv.likely)}</strong> → ${fmtEurFloat(rv.high)}</div>` +
        `<div class="gn-section"><span class="gn-label">Win prob:</span> ${Math.round((p.win_probability||0)*100)}%</div>` +
        `<div class="gn-section"><span class="gn-label">${t("Rekomandimi:")}</span> <strong>${htmlEsc(p.honest_recommendation||"")}</strong></div>` +
        `<div class="gn-section"><span class="gn-label">${t("Thuaji klientit:")}</span> ${htmlEsc(p.what_to_tell_client_today||"")}</div>`;
    }
  ...
  precHistEl?.addEventListener("change", async () => {
    ...
    try {
      ...
      precStatusEl.textContent = `Brief #${row.id} ${t("u ngarkua")} ✓`;
