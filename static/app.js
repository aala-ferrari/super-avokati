  ...
  function translateDOM(root) {
    ...
    try {
      ...
      for (var j = 0; j < els.length; j++) {
        ...
        if (ph && T_IT[ph.trim()]) els[j].setAttribute("placeholder", T_IT[ph.trim()]);
        var ti = els[j].getAttribute("title");
        if (ti && T_IT[ti.trim()]) els[j].setAttribute("title", T_IT[ti.trim()]);
      }
